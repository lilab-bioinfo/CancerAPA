...
option_list = list(
  ...
  make_option("--name", action="store", default=NA, type='character',
              help="blsmm prefix,tiss_transcript_id) [required]"),
...
weights.bslmm = function( input , bv_type , snp , out=NA ) {
	...
	out=strsplit(out,"/")[[1]][12]
	arg = paste( opt$PATH_gemma , " -miss 1 -maf 0.01 -r2 1 -rpace 1000 -wpace 1000 -bfile " , input , " -bslmm " , bv_type , " -o " , out , sep='' )
	print (arg)
