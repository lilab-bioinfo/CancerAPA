...
def is_number(s):
	try:
		...
		return False
def open_gz(filename,outputfile,reference_dict):
		...
		if len(line0)>=5:
			...
			if SNP in reference_dict:
				...
				if is_number(beta) and is_number(tstat):					
					...
					rsid = valueL[2].strip()
					MAF = valueL[3].strip()
					f_out.write(chrom+'\t'+position+'\t'+position+'\t'+gene+'/'+rsid+'/'+line0[4].strip()+'/'+beta+'/'+beta_se+'/'+MAF+'\n')
	aQTL.close()
	f_out.close()
	#return output
def main(args):
	...
	reference_dict = {}
	Allobfile = {}
	print_run("mkdir -p " + " "+"unmapaQTLs")
	print_run("mkdir -p " + " "+"bedQTLs")
#	print_run("mkdir -p " + " "+"aQTLs_postionTohg19")
...
#	output=outputfile+"/aQTLs_postionTohg19/"+name+ '.cis_aqtl_hg19.txt'
...
#			gene = other[0].strip()
#			rsid = other[1].strip()
#			pval_nominal = other[2].strip()
#			beta = other[3].strip()
#			varbeta = other[4].strip()
#			MAF = other[5].strip()
#			w.write(gene+"\t"+chromosome+"\t"+position+"\t"+rsid+"\t"+pval_nominal+"\t"+beta+"\t"+varbeta+"\t"+str(N)+"\t"+MAF+"\n")
#		print_run("rm " + " "  + temname)
#		print_run("mv " + " " + unlifted+ " " + "unmapaQTLs")
